    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if LEECH_LOG and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                ...
                logleechwarn = f'<b>\n⇛ This message will be deleted in {auto_delete_message} minutes from this group.\n⇛ Check <a href=\"{LEECH_LOG_URL}\">Log Channel.</a></b>\n'
        elif self.message.chat.type == 'private':
            logleechwarn = ''
        else:
            logleechwarn = ''
        ...
        if self.isLeech:
            ...
            if EMOJI_THEME is True:
                ...
                msg += f'\n<b>⇛ #Leeched: </b>{self.tag} | <b>ID: </b><code>{self.user_id}</code>\n'
            else: 
                msg += f'\n<b>├ It Tooks:</b> {get_readable_time(time() - self.message.date.timestamp())}'
                msg += f'\n<b>╰ #Leeched: </b>{self.tag} | <b>ID: </b><code>{self.user_id}</code>\n'
            ...
            if EMOJI_THEME is True:
                msg += f'\n<b>⇛ It Tooks • </b> {get_readable_time(time() - self.message.date.timestamp())}'
                msg += f'\n<b>⇛ #Mirrored: </b>{self.tag} | <b>ID: </b><code>{self.user_id}</code>\n'
            else:
                msg += f'\n<b>├ It Tooks:</b> {get_readable_time(time() - self.message.date.timestamp())}'
                msg += f'\n<b>╰ #Mirrored: </b>{self.tag} | <b>ID: </b><code>{self.user_id}</code>\n'
            buttons = ButtonMaker()
